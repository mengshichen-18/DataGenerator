...
class CoreWorkload {
 public:
  ...
  static const std::string HIT_RATE_PROPERTY;
  static const std::string HIT_RATE_DEFAULT;
...
inline std::string CoreWorkload::NextSequenceKey() {
  // uint64_t seq_id_ = int(key_generator_->Next() % key_generator_->max_);
  // std::cout<<"Max: "<< key_generator_->max_<<" "; 
  uint64_t seq_id_ = key_generator_->Next();
  insert_key_sequence_.Next();
  StringKey key;

  if(from_file_) {
    // std::cout<<"Seq ID: "<<seq_id_<<endl;
    ...
    key = keys_[seq_id_];       // a key from given dataset
      ...
      // std::cout<<"Seq ID: "<<seq_id_<<" key: "<<key.str_<<endl;
    #endif
  }
