...
void ParseCommandLine(int argc, const char *argv[], utils::Properties &props) {
    ...
    while (argindex < argc) {
        if (strcmp(argv[argindex], "-P") == 0) {
            ...
            props.SetProperty("output_file", argv[argindex]);
            argindex++;
        } 
        else {
...
int main(const int argc, const char *argv[]) {
    ...
    const string ofilename = props.GetProperty("output_file", "");
    ...
    const string dist = props.GetProperty(CoreWorkload::REQUEST_DISTRIBUTION_PROPERTY, "0");

    ycsbc::CoreWorkload wl(filename, insertstart + recordcount + operationcount * (insert_ratio + 0.1));
    wl.Init(props);

    string filetag = filename;
    filetag = filetag.substr(0, filetag.length() - 4);
    
    // generate load
    // BasicDB db_load(ofilename+ "_datset.dat");
    std::string key;
    ...
    BasicDB db_txn(ofilename + "_query.dat");
