...
const string CoreWorkload::MAX_SCAN_LENGTH_DEFAULT = "1000000";
...
const string CoreWorkload::HIT_RATE_PROPERTY = "hitrate";
const string CoreWorkload::HIT_RATE_DEFAULT = "50";
...
void CoreWorkload::Init(const utils::Properties &p) {
  ...
  int hit_rate = std::stoi(p.GetProperty(HIT_RATE_PROPERTY,
                                                HIT_RATE_DEFAULT));
  ...
  if (request_dist == "uniform") {
    std::cout<<"Insert start: "<<insert_start<<" record_count_: "<<record_count_<<endl;
    key_chooser_ = new UniformGenerator(0, insert_start + record_count_ - 1);
    // key_chooser_ = new UniformGenerator(0, insert_start - 1);
    // key_chooser_ = new UniformGenerator(0, 4);
  } else if (request_dist == "zipfian") {
    std::cout<<"Hi I'm zipfian!"<<endl;
    ...
    std::cout<<"Num: "<<insert_start + record_count_ + new_keys<<endl;
    key_chooser_ = new ScrambledZipfianGenerator(insert_start + record_count_ + new_keys, zipfian_skewness);
    // key_chooser_ = new ZipfianGenerator(insert_start + record_count_ + new_keys, zipfian_skewness);
  ...
  if (scan_len_dist == "uniform") {
    std::cout<<"Max scan len: "<<max_scan_len<<endl;
    scan_len_chooser_ = new UniformGenerator(1, max_scan_len);
    // scan_len_chooser_ = new UniformGenerator(1, 2);
